...
from helper import vibrating, fileExist, distance
...
MIN_BR=10
UTC_ZONE=3
ALARM_WAV='res/ping.wav'
NOTIFY_WAV='res/link.wav'
NOTIFY_PERIODIC=[-1,-1]
wavFreez = False
showEnbl = False
...
def ConfigLoad():
  ...
  try:
    ...
    NOTIFY_WAV=data['setup']['NOTIFY_WAV']
    NOTIFY_PERIODIC=data['setup']['NOTIFY_PERIODIC']
...
def showAudio():
  ...
  def playAlarm2():
    ...
    wavFreez = False
  def event_play_handler(obj, event):
    if event == lv.EVENT.CLICKED:
      if wavFreez == False:  
        if imgbtn1==obj:
  ...
  ddlist.set_size(220,30)
  ddlist.set_selected(wavs_ind)
  ddlist.set_event_cb(event_list_handler)
  imgbtn1 = lv.imgbtn(page, None)
  imgbtn1.set_src(lv.btn.STATE.RELEASED, loadPNG("res/play_icon.png"))
  imgbtn1.set_checkable(True)
  imgbtn1.set_pos(265, 45)
  imgbtn1.set_event_cb(event_play_handler)
  label=lv.label(page,None)
  ...
  for i,d in enumerate(wavs):
    if d in NOTIFY_WAV:
      wavs_ind=i
      break
  ddlist2 = lv.dropdown(page)
  ddlist2.set_options("\n".join(wavs))
  ddlist2.set_pos(30,115)
  ddlist2.set_size(220,30)
  ddlist2.set_selected(wavs_ind)
  ddlist2.set_event_cb(event_list_handler)
  imgbtn2 = lv.imgbtn(page, None)
  imgbtn2.set_src(lv.btn.STATE.RELEASED, loadPNG("res/play_icon.png"))
  imgbtn2.set_checkable(True)
  imgbtn2.set_pos(265, 115)
  imgbtn2.set_event_cb(event_play_handler)
  lv.disp_load_scr(subscreen)
...
def showSystem():
  global subscreen
  def setLed():
    power.setPowerLED(deviceCfg.get_power_led_mode())
  def cl_reboot():
